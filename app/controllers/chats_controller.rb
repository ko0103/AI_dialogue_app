...
class ChatsController < ApplicationController
  ...
  def create
    ...
    user_message = Message.new(user: user, chat_session: chat_session, content: message)
    ...
    if user_message.save
      ...
      # モデルのバリデーションエラーを表示
      flash.now[:alert] = user_message.errors.messages[:content]&.join("<br>")
      render turbo_stream: turbo_stream.update("chat-form", partial: "chats/chat_form") and return
    end
  end
end
